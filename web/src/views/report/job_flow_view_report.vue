<template>
    <div id="jobFlowViewReport">
        <div class="search-header">
            ...
            <div class="search-item" style="width: 20%;">
                <span style="flex-basis: 70px;">作业流名称</span>
                <bk-select :clearable="true" style="background-color: #fff;flex: 1;" placeholder="请选择" searchable
                    ...
                    <bk-option v-for="(item, index) in jobFlowNameList" :key="index" :id="item" :name="item"
                        v-if="jobFlowNameShow">
                    </bk-option>
                </bk-select>
            </div>
            <div class="search-item" style="width: 38%;">
                ...
                <bk-date-picker :placeholder="'选择日期时间'" :type="'datetimerange'" format="yyyy-MM-dd HH:mm:ss"
                    :transfer="true" :value="searchForm.timeRange" @change="handleTimeRangeChange" style="flex: 1;" :title="searchForm.timeRange"></bk-date-picker>
            </div>
            ...
            <!--<bk-form form-type="inline" ext-cls="custom-form">
                ...
                <bk-form-item label="作业流名称" style="margin-left: 12px;">
                    <bk-select :clearable="true" style="background-color: #fff;width: 240px;" placeholder="请选择" searchable v-model="searchForm.name">
                        <bk-option v-for="(item, index) in jobFlowNameList" :key="index" :id="item" :name="item" v-if="jobFlowNameShow">
                        </bk-option>
                    </bk-select>
                </bk-form-item>
                <bk-form-item label="时间范围" style="margin-left: 12px;">
                    <bk-date-picker :placeholder="'选择日期时间'" :type="'datetimerange'" format="yyyy-MM-dd HH:mm:ss"
                        :transfer="true" :value="searchForm.timeRange" @change="handleTimeRangeChange"></bk-date-picker>
                </bk-form-item>
        ...
        <div class="chart1" v-bkloading="{ isLoading: chart1Loading, zIndex: 99 }">
            <div class="title">作业流执行情况分析图</div>
            <div class="chart-name">
                <span>作业流数（个）</span>
                <span>作业流成功率 (%) </span>
            </div>
            <div :id="jobFlowReportChart1Id" style="height: 290px;margin-top: 20px;"></div>
        </div>
        ...
        <div class="chart2" v-bkloading="{ isLoading: chart2Loading, zIndex: 99 }">
            ...
            <div class="title">作业流性能分析图</div>
            <div :id="jobFlowReportChart2Id" style="height: 290px;margin-top: 20px;"></div>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                midJobFlowName: '所有作业流',
                jobFlowNameShow: true,
                searchForm: {
                    category: '', // 跑批id
                    name: '', // 作业流名称
                    timeRange: ['', ''] // 时间范围
        ...
        computed: {
            jobFlowReportChart1Id() {
                return 'jobFlowReportChart1' + this._uid
            },
            jobFlowReportChart2Id() {
                return 'jobFlowReportChart2' + this._uid
            }
        },
        created() {
            this.getJobFlowName()
        ...
        methods: {
            ...
            // 获取作业流下拉列表
            getJobFlowName() {
                this.jobFlowNameShow = false
                this.jobFlowNameList = []
                this.$api.processReport.getJobFlowName({
                    category: this.searchForm.category
                }).then(res => {
                    if (res.result) {
                        this.jobFlowNameList = res.data
                    } else {
                        this.$cwMessage(res.message, 'error')
                    }
                    this.jobFlowNameShow = true
                })
            },
            // 时间范围格式转换
            ...
            // 处理初始化报表
            initChart() {
                this.chart1 = this.$echarts.init(document.getElementById(this.jobFlowReportChart1Id))
                this.chart2 = this.$echarts.init(document.getElementById(this.jobFlowReportChart2Id))
            ...
            handleLoad() {
                ...
                this.$api.processReport.getJobFlowReport(params).then(res => {
                    if (res.result) {
            ...
            handleRenderChart1(chart1Data) {
                ...
                this.chart1Option.series[0].data = chart1Data.day_process_count // chart1作业流总数
                this.chart1Option.series[1].data = chart1Data.day_success_process_count // chart1成功作业流数
                this.chart1Option.series[2].data = chart1Data.day_error_process_count // chart1异常作业流数
                this.chart1Option.series[3].data = chart1Data.day_confirm_process_count // chart1人工复核作业流数
                this.chart1Option.series[4].data = chart1Data.day_rerun_process_count // chart1重执行作业流数
                this.chart1Option.series[5].data = chart1Data.day_process_success_rate // chart1作业流执行成功率
                const max = this.cal_Max([...chart1Data.day_process_count, ...chart1Data.day_success_process_count, ...chart1Data.day_error_process_count, ...chart1Data.day_confirm_process_count, ...chart1Data
                    .day_rerun_process_count
                ])
            ...
            handleRenderChart2(chart2Data) {
                ...
                // this.chart2Option.yAxis.max = parseInt(max) + 100
                // this.chart2Option.yAxis.interval = (parseInt(max) + 100) / 5
                ...
                const option = {
                    ...
                    tooltip: {
                        ...
                        formatter: function(params, ticket, callback) {
                            let htmlStr = '' + '<strong>' + _this.midJobFlowName + '</strong>' + '<br/>'
                            for (let i = 0; i < params.length; i++) {
                                ...
                                htmlStr += '<div style="margin-top: 6px;">' +
                                    ...
                                    color + ';"></span>'
                                // 圆点后面显示的文本
                                htmlStr += seriesName + '：' + value + (seriesName === '作业流执行成功率' ? '%' : '') +
                                    '</div>'
                                htmlStr += '</div>'
                            }
                            return htmlStr
                    ...
                    legend: {
                        data: ['作业流总数', '成功作业流', '异常作业流', '人工复核作业流', '重执行作业流', '作业流执行成功率'],
                        bottom: 'bottom',
                        // itemWidth: 12,
                        itemHeight: 12,
                        textStyle: {
                        ...
                        formatter: function(params, ticket, callback) {
                            ...
                            for (let i = 0; i < params.length; i++) {
                                ...
                                htmlStr += '<div style="margin-top: 6px;">' +
                                    ...
                                    color + ';"></span>'
                                // 圆点后面显示的文本
                                // htmlStr += seriesName + '：' + (value === undefined ? '' : value) + '</div>'
            ...
            handleChartResize() {
                ...
                this.$nextTick(() => {
                    ...
                    // 监听元素 大小变化
                    erd.listenTo(document.getElementById('jobFlowViewReport'), function(element) {
                        _this.chart1.resize()
                        _this.chart2.resize()
                    })
                })
            ...
            handleSearch() {
                if (this.searchForm.name === '') {
                    this.midJobFlowName = '所有作业流'
                } else {
                    this.midJobFlowName = this.searchForm.name
                }
                this.handleLoad()
            },
            // 处理重置
