...
class _Pickler:
    ...
    def save_pers(self, pid):
        ...
        if self.bin:
            yield self.save(pid, save_persistent_id=False)
    ...
    def save_bytes(self, obj):
        if self.proto < 3:
            if not obj:  # bytes object is empty
                yield self.save_reduce(bytes, (), obj=obj)
            else:
                yield self.save_reduce(codecs.encode, (str(obj, "latin1"), "latin1"), obj=obj)
            return
        n = len(obj)
        if n <= 0xFF:
    ...
    def save_dict(self, obj):
        ...
        self.memoize(obj)
        yield self._batch_setitems(obj.items())
    ...
    def save_global(self, obj, name=None):
        ...
        try:
            ...
            raise PicklingError("Can't pickle %r: it's not found as %s.%s" % (obj, module_name, name))
        else:
            if obj2 is not obj:
